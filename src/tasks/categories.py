...
class Categories(Dataset):
    ...
    def set_instance_feature_matrix(self, data_matrix, data_index_dict):
        ...
        for i in range(self.num_instances):
            ...
            if instance in data_index_dict:
                index = data_index_dict[instance]
                embedding = data_matrix[index, :]
    ...
    def create_similarity_matrix(self, metric: str):
        if metric == "correlation":
            self.similarity_matrix = np.corrcoef(self.instance_feature_matrix)
        elif metric == "cosine":
            self.similarity_matrix = distance.squareform(distance.pdist(self.instance_feature_matrix, "cosine"))
        else:
