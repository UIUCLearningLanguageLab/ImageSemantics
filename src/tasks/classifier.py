...
class Classifier(nn.Module):
    ...
    def set_params(self, train_params_dict):
        self.required_params_list = ['classifier_hidden_sizes', 'classifier_num_epochs', 'classifier_learning_rate',
                                     ...
                                     'classifier_optimizer', 'classifier_criterion', 'classifier_device', 'verbose']
        ...
        for fold, (train_idx, test_idx) in enumerate(kf.split(range(len(self.categories)))):
            ...
            for model_num in range(self.num_classifiers):
                ...
                for epoch in range(self.classifier_num_epochs):
                    ...
                    train_results = self.test_model(train_loader, epoch+1, fold, model_num, "train")
                    test_results = self.test_model(test_loader, epoch+1, fold, model_num, "test")
                    ...
                    if self.verbose:
                        train_sum = 0
                        test_sum = 0
                        for i, instance in enumerate(train_results):
                            train_sum += instance['correct']
                        for i, instance in enumerate(test_results):
                            test_sum += instance['correct']
                        train_pc = train_sum/len(train_results)
                        test_pc = test_sum/len(test_results)
                        print(f"{epoch}-{train_pc:0.3f}-{test_pc:0.3f}")
